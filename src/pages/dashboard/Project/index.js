    ...
    <React.Fragment>
      ...
      {(() => {
        switch (itemsmenuprincipal) {
          case 'Material':case 'ManoObra':case 'Herramientas':case 'Cliente': case 'Proyecto':
            return (<><GestionBasica
               accion={'GestionBasica'}
               tipo={itemsmenuprincipal}
               /></>);
          case 'RegistrarEmpleado':case 'ControlDiario': case 'OrdenCompra':case 'GenerarNomina':case 'CotizarProyecto':case 'LiquidarProyecto':case 'EstadoProyecto':
            return (<>{''}</>);
